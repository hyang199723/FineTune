...
#ref_image
# Load reference mask
fn = wk_dir + "Labels/SolarPanelPNG/" + "Frame_001_2.png"
ref_mask = cv2.imread(fn)
...
#%% Test Image
output_dir = "Outputs/PerSAM_1_2_f/"
N = 275
...
for i in range(1,N+1):
    filename = wk_dir + "Data/SolarPanel/Frame_" + f"{i:03}" + ".png"
    ...
    axes.title.set_text(f"Predicted mask on gt 001_1") 
    axes.scatter(topk_xy[0][0], topk_xy[0][1], color='green', marker='*', 
                 s=50, edgecolor='white', linewidth=1.25)
    axes.axis("off")
    fig.savefig(wk_dir + output_dir + "Frame_" + f"{i:03}" + ".png")
# %%
